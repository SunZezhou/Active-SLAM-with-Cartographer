...
class robot:
	...
	def __init__(self,name):
		...
		self.listener.waitForTransform(self.global_frame, 'base_footprint', rospy.Time(0),rospy.Duration(10.0))
		cond=0
		while cond==0:	
			try:
				(trans,rot) = self.listener.lookupTransform(self.global_frame, 'base_footprint', rospy.Time(0))
				cond=1
			except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
				cond==0
		rospy.loginfo('tf gotten.')
		self.position=array([trans[0],trans[1]])		
		self.assigned_point=self.position
		self.client=actionlib.SimpleActionClient(self.name+'/move_base', MoveBaseAction)
		rospy.loginfo('try connect actionlib server.')
		self.client.wait_for_server()
		rospy.loginfo('actionlib connected.')
